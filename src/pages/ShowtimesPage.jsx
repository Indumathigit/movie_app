          ...
          <div className="flex flex-wrap gap-2">
            ...
            {theaters.map(function (t) {
              return (
                <button
                  key={t.id}
                  onClick={() => setSelectedTheaterId(String(t.id))}
                  className={`text-sm px-4 py-2 rounded-xl border ${
                    selectedTheaterId === String(t.id)
                      ? "bg-red-600 border-red-600 text-white"
                      : "bg-gray-900 border-gray-800 text-gray-400 hover:border-gray-600"
                  }`}
                >
                    ...
                    {theaterShowtimes.map(function (show) {
                      return (
                        <button
                          ...
                          className="group bg-gray-800 hover:bg-red-600 border border-gray-700 hover:border-red-600 rounded-xl p-3 text-left min-w-[140px]"
                        >
                          <p className="text-white font-bold text-lg mb-1">{show.time}</p>
                          <p className="text-gray-400 group-hover:text-red-200 text-xs mb-2">
                            {show.format} • {show.screen}
                          </p>
                          <p className="text-green-400 text-xs font-medium mb-1">
                            From ₹{show.price.standard}
                          </p>
                          <p className={`text-xs font-medium ${getAvailabilityColor(show.availableSeats)}`}>
                            {getAvailabilityLabel(show.availableSeats)} • {show.availableSeats} seats
                          </p>
                        </button>
                      )
                    })}
        ...
        <div className="mt-8 bg-gray-900 border border-gray-800 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-white text-sm font-medium">Recliner</p>
                <p className="text-gray-400 text-xs">Rows A-B • ₹450+</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-400 text-xs">Rows C-E • ₹280+</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-4 h-4 rounded bg-gray-500 flex-shrink-0" />
              <div>
                <p className="text-white text-sm font-medium">Standard</p>
                <p className="text-gray-400 text-xs">Rows F-H • ₹180+</p>
              </div>
            </div>
          </div>
        </div>
